...
## 4. 网页侧验收（2026-09-22，派发 `w34-web`）

派发跑完（2211s），自己跑过 lint / typecheck / vitest（377 条）与 5 条自出的变异，F4 基线 324 → 308（删 16 个 `kunFetch` 调用点，新增 0）。

**验收修的**：

- **`reportProblem` 只渲染本地化的原因，从不说是哪个字段**。这一轨新加了「逐条 toast 字段错误」的循环，于是一个 422 会弹出「内容过短」「选择的项太少」两条，用户不知道是哪一项。改成复用现成的 `KUN_FIELD_LABELS`，与客户端 Zod 报错同一个「标签：消息」格式，`parameter` / `header` 错误也带上名字。
- 删掉两个零引用的旧类型文件（沙箱的删除工具拒绝了它，所以它只能在报告里列出来）。
- 删掉 `kunMessage.ts` 里写着「萌萌点不足 20」的死文案——推的价格是 10，而且这个码在 Go 侧根本不存在。

**变异**：我出的 15 条题，第一轮杀 10 条。**存活的 4 条全是同一个洞**：K16 的「撤销」那一半——取消收藏、取消置顶、取消设置最佳答案的 `DELETE` 分支，网页侧一条测试都没有，把三处 `DELETE` 全改成 `PUT` 照样全绿。补了 `topic/unset.spec.ts` 覆盖三个撤销分支，外加 `reportProblem.spec.ts`。补完 15/15 全杀。

**浏览器实测**（dev 栈，临时会话，用完已删；话题 4121）：

| 操作 | 结果 |
|---|---|
| 收藏 / 取消 | `PUT` → 200，计数 10→11；`DELETE` → 200，回到 10 |
| 推 | `POST /upvotes` → 201，行落库、`updated` 有值、备注首尾空白被去掉、计数 1、推者缓存余额 1143→1133 |
| 表情 置位 / 撤销 | `PUT` / `DELETE /reactions/sob` → 200，汇总 14→15→14 |
| 表情历史 | `GET /topics/4121/reactions?limit=20` → 200，17 人新到旧 |
| 楼主能力 | `viewer` 给出 `can_edit` / `can_hide` / `can_set_best_answer` / `can_pin_reply`，`can_like` / `can_upvote` 为假（自己的话题） |
| 重新编辑 | `GET /topics/4121/source` → 200，标题与正文回填 |

弹窗文案已经是「消耗您 10 萌萌点」（旧的错误文案写的是 20）。

dev 库里留下了一条备注为 `W34 browser check` 的推记录（话题 4121），推不可撤销，是有意留下的测试痕迹。
