...
## 5.9 生产取值（督查 2026-09-22 实测，生产库 `kungalgame`）

§6 列的问题，答案在这里。**几个数字直接改变了优先级排序。**

| 问题 | 值 | 结论 |
|---|---|---|
| §6.1 `topic_poll.status` | **35 行全是 `open`** | `closed` 从未被写过，`status` 确是死轴，v1 直接不要这个字段 |
| §6.2 `result_visibility` / `is_anonymous` | always 31（10 匿名）/ after_vote 4（2 匿名）；无 after_deadline | 两个枚举成员都在用，匿名投票占 1/3，去匿名风险是真的 |
| §6.3 跨投票投票 | **0** | 我 2026-09-22 修掉的那个洞从未被利用 |
| §6.4 `vote_count` 漂移 | **0** | 不需要重算 |
| §6.5 空选项 / 超长选项 | **0 / 0** | 缺 `dive` 至今没被踩到，是潜伏缺陷不是现存脏数据 |
| §6.7 抽奖状态 | **只有 2 个抽奖，都是 `drawn`**；无卡死的 `drawing`；无 `open + deadline IS NULL` | §2.2「永远开不了奖」没有发生 |
| §6.9 抽奖凭空铸币 | **350 萌萌点，5 个用户** | §2.1 属实，但规模很小 |
| §6.10 托管兑换码 | **0 条** | 生产从未用过 code 奖项，所以启动日志里 `KUN_LOTTERY_CODE_KEY 未设置` 的告警至今无后果 |
| §6.11 带图奖品 | 1 个 | 图被 GC 的风险面是 1 行 |
| §6.12 草稿 | **69 条 / 57 人**；顶到 30 条上限的：0 | 30 条上限从未挡住任何人 |
| §6.13 抽奖迁移 083–087 | **全部已应用** | 功能在生产上是存在的，只是几乎没人用 |
| §6.15 孤儿投票 | **0** | 外键还在 |

**排序结论**：投票 35 个、草稿 69 条、抽奖 2 个。**抽奖 11 个端点服务 2 行数据**，优先级应当排在投票和草稿之后；先做投票（6 端点）与草稿（4 端点）。
