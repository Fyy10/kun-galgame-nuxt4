...
| 写面在 OAuth 抖动时 503 | **属实，且是真实的可用性耦合变化**。v1 每个写都先过 `visibleTopic` → 封禁作者闸 → `lookupUsers`，上游报错即 `problem.Unavailable`（503）；旧写面根本不碰 userclient，旧读面用 `Hydrate` 吞错回占位符 | **未动，等你定**：失败时「关闭」在安全上说得通，但这条没在裁决里声明过 |

另外核实的两条：

- **`PATCH /topics/{id}` 只在标题或正文变了时才跑 trust 检查**——属实。旧 `PUT` 每次编辑都跑。新行为说得通（没有新文本就没什么可查），但确实未声明。
- **推的备注先按原串校验 30 长度、再去首尾空白**，与裁决写的「先去空白再判超长」相反。网页把输入裁到 30 个字符（含空白），所以**从产品自己的客户端走不到**；改已发布的 `maxLength` 属于契约决定，记录不改。
