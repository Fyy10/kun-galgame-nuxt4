...
|---|---|---|---|---|
| **T** | **话题**：`/topic/**` 27 + `/admin/topic*` 3 + `/resource` 1 | **31** | 110–119 | 已认领（本轨）。四段串行：**T1** 清理旧评论/投票 10 条 → **T2** 草稿 4 + `interactions/mine` + `reply/locate` + 删 `/resource` 共 7 条 → **T3** 抽奖 11 条 → **T4** 管理面 3 条（在 `internal/admin/**`，页码集合） |
