...
type Comment struct {
	...
	InReplyToUser   repr.UserRef    `json:"in_reply_to_user" doc:"The user the comment answers: the parent comment's author, or the reply's author for a top-level comment."`
	Text            string          `json:"text" maxLength:"1007" doc:"Comment text. Plain text, not Markdown; render it as text. Free text; never use it as a decision input."`
