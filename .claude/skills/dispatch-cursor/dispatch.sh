...
deny=(
  ...
  'Shell(*env-llm-key*)'
  # This repo's six env files include docker/{api,web}.env and two .env.prod files, which the
  # two .env rules above do not match.
  'Shell(*.env.prod*)'    'Shell(*api.env*)'       'Shell(*web.env*)'
...
env_fence=(
  ...
  -u TEST_DATABASE_DSN -u DATABASE_URL -u PGPASSWORD -u PGHOST -u PGUSER -u KUN_DATABASE_URL
  SHELL=/bin/bash
  ZDOTDIR="$out/zdotdir"
